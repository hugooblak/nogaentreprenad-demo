...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'nogaentreprenad-leads' ),
					'singular_name' => __( 'Lead', 'nogaentreprenad-leads' ),
					'edit_item'     => __( 'Förfrågan', 'nogaentreprenad-leads' ),
					'search_items'  => __( 'Search leads', 'nogaentreprenad-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'nogaentreprenad-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'nogaentreprenad-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'nogaentreprenad-leads' ),
			'npl_phone'    => __( 'Phone', 'nogaentreprenad-leads' ),
			'npl_service'  => __( 'Needs', 'nogaentreprenad-leads' ),
			'npl_timeline' => __( 'When', 'nogaentreprenad-leads' ),
			'npl_zip'      => __( 'ZIP', 'nogaentreprenad-leads' ),
			'npl_source'   => __( 'Source', 'nogaentreprenad-leads' ),
			'npl_status'   => __( 'Status', 'nogaentreprenad-leads' ),
			'date'         => __( 'Received', 'nogaentreprenad-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'nogaentreprenad-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'nogaentreprenad-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'nogaentreprenad-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'nogaentreprenad-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'nogaentreprenad-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'nogaentreprenad-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'nogaentreprenad-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'nogaentreprenad-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'nogaentreprenad-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'nogaentreprenad-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'nogaentreprenad-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'nogaentreprenad-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
