...
add_filter(
	...
	function ( $exporters ) {
		$exporters['nogaentreprenad-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'nogaentreprenad-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'nogaentreprenad-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'nogaentreprenad-leads',
						'group_label' => __( 'Quote requests', 'nogaentreprenad-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['nogaentreprenad-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'nogaentreprenad-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
